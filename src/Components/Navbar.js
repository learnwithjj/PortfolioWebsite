...
//    },[darkMode]);
    ...
    return(
        <div  style={{fontSize:"20px",display:"flex"}}>
              ...
              {/* { window.location.pathname==="/about"?<div></div>: <div className="rightdiv"><button id="darkmodebutton" onClick={()=>setDarkMode(!darkMode)}>{!darkMode? <HiMoon/>:<HiLightBulb/>}   {!darkMode? "Dark Mode" :"Light Mode"}</button></div>} */}
        </div>
    );
}
